...
function TagSection({ title, hint, options, selected, choose }) { return <Card><div className="between"><div><h3>{title}</h3><p className="muted">{hint}</p></div><span className="badge">Pick one</span></div><div className="pills">{options.map(o => <Pill key={o} on={selected[0] === o} click={() => choose(o)}>{o}</Pill>)}</div></Card>; }
